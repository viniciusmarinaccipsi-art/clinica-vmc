...
Pseudo-elementos que exibiam emoji (`.card.locked …::after`, `.auto-sec-warn::before`; `.auto-group-summary::before` saiu no 16.5d-2) usam máscara (`--ico-lock`, `--ico-check`, `--ico-check-square`, `--ico-alert`, declaradas no `:root`) com `background:currentColor`.
...
**Pacote 16.5d-2 (etapas):** nenhum símbolo novo — os ícones do cabeçalho cumulativo (`#i-pin`, `#i-heart-crack`/`#i-heart`, `#i-zap`/`#i-leaf`, `#i-cloud-rain`/`#i-sun`, `#i-trend-down`/`#i-trend-up`, `#i-humor-N`, `#i-pencil` em "editar", `#i-layers` em "ver tudo") já existiam no sprite e batem com o `icones.js` da rodada 3. Usos novos: check redondo da lista de subgrupos (`#i-check`, 16 px, traço 2,5, em `.auto-check`), aviso de obrigatório na tela (`#i-alert`, 20 px, em `.auto-aviso`) e cartão de ajuda (`#i-bulb`, 24 px). O cabeçalho pinta o rostinho do humor com a tinta do tipo de registro (`[data-tipo] .cab-linha .ico`), sobrepondo `.ico-humor-N`.
