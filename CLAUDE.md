...
**Redesenho (`index-dev.html`, tema G):** `docs/design/tokens.css` no lugar do `:root` antigo — 99 tokens claros + 58 escuros (`prefers-color-scheme: dark`); **fonte única Figtree** via Google Fonts (400/500/600/700) — `--f-title` e `--f-text` são a mesma família, títulos/escores/horas em 600 e texto em 400; a Newsreader saiu no Pacote 16.4.5 (nenhuma serifa no app; sem `<link rel="manifest">`); **cor por significado** — risco/erro só vermelho (`--c-risk*`), Registro Negativo ameixa (`--c-reg-neg*`), Positivo verde-água (`--c-reg-pos*`), ação índigo (`--c-action*`), atenção âmbar (`--c-warn*`), um par tinte/tinta por instrumento (`--c-phq9-*` … `--c-srq20-*`); texto usa a variante `-ink`; sombras `--sh-1/2/3/bar`, raios `--r-*`, toque `--hit-min`/`--hit-likert`; ícones pelo sprite SVG inline `#i-*` (`docs/design/icones.md`), emoji só nas 5 faces da escala de humor. Nenhuma cor fixa fora dos `:root`.
...
- `docs/licoes-aprendidas.md` — lições 1–84 (numeração original; obsoletas marcadas). Lições novas nascem na seção 8 do status e entram aqui na regeneração seguinte da skill.
